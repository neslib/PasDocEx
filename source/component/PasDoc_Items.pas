...
{ @abstract(defines all items that can appear within a Pascal unit's interface)
  @created(11 Mar 1999)
  @cvs($Date: 2015-01-03 23:54:08 +0100 (sob) $)
...
begin
  ...
  if (Result = 0) then
  begin
    { Move methods with descriptions to the top }
    if (TPasItem(PItem1).ClassType = TPasMethod)
      and (TPasItem(PItem2).ClassType = TPasMethod)
      and (TPasItem(PItem1).HasDescription <> TPasItem(PItem2).HasDescription) then
    begin
      if (TPasItem(PItem1).HasDescription) then
        Result := -1
      else
        Result := 1;
    end;
...
var Ancestor, NewAncestor: TBaseItem;
begin
  ...
  begin
    ...
    NewAncestor := TPasCio(Ancestor).FirstAncestor;

    { There can be rare situations where Ancestor equals itself, for
      example when using generics:

        TFoo = class...
        TFoo<T> = class(TFoo)...

      PasDoc will parse this incorrectly and set the ancestor of TFoo to TFoo!. }
    if (NewAncestor = Ancestor) then
      Exit(nil);
...
begin
  ...
  begin
    { When a raises name ends with a ":", then remove that colon }
    if (Pair.Name.EndsWith(':')) then
      SetLength(Pair.Name, Pair.Name.Length - 1);
    FRaises.Add(Pair);
  end;
...
begin
  ...
  begin
    { When a parameter name ends with a ":", then remove that colon }
    if (Pair.Name.EndsWith(':')) then
      SetLength(Pair.Name, Pair.Name.Length - 1);
    FParams.Add(Pair);
  end;
...
constructor TPasProperty.Create;
begin
  inherited;
  FParams := TStringPairVector.Create(true);
  FRaises := TStringPairVector.Create(true);
...
procedure TPasProperty.RegisterTags(TagManager: TTagManager);
begin
  { properties may also have parameters (indexes properties) and may
    also raise exceptions. So we need to support these tags. }
  inherited;
  TTopLevelTag.Create(TagManager, 'raises',
    nil, {$IFDEF FPC}@{$ENDIF} StoreRaisesTag,
    [toParameterRequired, toRecursiveTags, toAllowOtherTagsInsideByDefault,
...
procedure TPasProperty.StoreParamTag(ThisTag: TTag; var ThisTagData: TObject;
  EnclosingTag: TTag; var EnclosingTagData: TObject; const TagParameter: string;
...
procedure TPasProperty.StoreRaisesTag(ThisTag: TTag; var ThisTagData: TObject;
  EnclosingTag: TTag; var EnclosingTagData: TObject; const TagParameter: string;
  var ReplaceStr: string);
var
  Pair: TStringPair;
...
procedure TPasProperty.StoreReturnsTag(ThisTag: TTag; var ThisTagData: TObject;
  EnclosingTag: TTag; var EnclosingTagData: TObject; const TagParameter: string;
  var ReplaceStr: string);
begin
  if TagParameter = '' then exit;
