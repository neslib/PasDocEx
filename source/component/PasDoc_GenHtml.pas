...
{ @abstract(Provides HTML document generator object.)
  ...
  @cvs($Date: 2015-08-09 05:55:41 +0200 (nie) $)
...
begin
  ...
  begin
    { Write empty paragraphs so links to anchors towards the end of the page scroll
      better into view }
    WriteDirect('<div style="height:1000px"></div>');
...
begin
  ...
  begin
    ...
    { In case of following function overloads that don't have a description,
      merge those with this item }
    if (Item.ClassType = TPasMethod) then
    begin
      Inc(I);
      while (I < Items.Count) do
      begin
        NextItem := Items.PasItemAt[I];
        if (NextItem.HasDescription) or (NextItem.ClassType <> TPasMethod)
          or (NextItem.UnitRelativeQualifiedName <> Item.UnitRelativeQualifiedName)
        then
          Break;

        WriteItemTableRow(NextItem, ShowVisibility, false, true);
        Inc(I);
      end;
      Dec(I);
    end;
...
procedure TGenericHTMLDocGenerator.WriteItemLongDescription(
  ...
  procedure WriteParamsOrRaises(Func: TPasItem; const Caption: TTranslationID;
  ...
  begin
    ...
    begin
      ...
      begin
        { don't warn if exception type cannot be found... }
        ParamName := SearchLink(ParamName, Func, '', False);

        { ...but then ParamName is empty. So fix it. }
        if (ParamName = '') then
          ParamName := CodeString(ConvertString(List[i].Name));
    end;
  ...
  procedure WriteReturnDesc(Func: TPasItem; ReturnDesc: string);
  begin
    if ReturnDesc = '' then
      exit;
...
begin
  ...
  if AItem is TPasProperty then
  begin
    AItemProperty := TPasProperty(AItem);
    WriteParamsOrRaises(AItemProperty, trParameters,
      AItemProperty.Params, false, 'parameters');
    WriteReturnDesc(AItemProperty, AItemProperty.Returns);
    WriteParamsOrRaises(AItemProperty, trExceptionsRaised,
      AItemProperty.Raises, true, 'exceptions_raised');
  end;
